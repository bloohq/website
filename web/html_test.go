...
// TestHTMLServiceRenderHTMLPage tests rendering HTML pages
func TestHTMLServiceRenderHTMLPage(t *testing.T) {
	testDir := t.TempDir()
...
<p>This is a test page with {{.CustomerNumber}} customers.</p>`)
	...
	// Render HTML page
	html, err := htmlService.renderHTMLPage(filepath.Join(pagesDir, "test.html"), "/test", navigationService, seoService)
	if err != nil {
